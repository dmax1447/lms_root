...
;(function () {
  ...
  AreaPanel.prototype = {
    ...
    createSections(sections) {
      ...
      const sectionOverlapContainer = document.createElement('div')
    ...
    appendSection(section, sectionContainer, dropdownList, isActive) {
      const buttonSection = new ButtonSection(section, this)
    ...
    cloneButtonsToAreaBox(section) {
      ...
      for (let i = 0; i < section.buttons.length; i++) {
        const sectionButton = section.buttons[i]
        const newButton = new Button(sectionButton.id, section)
    ...
    scrollDownAreaBox() {
      const height = parseInt(this.panelAreaBox.scrollHeight)
      const marginTop = this.getMarginTop()
  ...
  ButtonPanel.prototype = {
    ...
    showSections(event) {
      ...
      if (finalWidth > document.body.clientWidth) {
        this.sectionsMountPoint.style.left =
          -this.sectionWidth + this.buttonElement.clientWidth + 'px'
      } else {
    ...
    appendSection(section, sectionDiv) {
      const buttonSection = new ButtonSection(section, this)
      if (buttonSection.buttons.length > 0) {
...
;(function () {
  ...
  ButtonSection.prototype = {
    createButtons(buttons) {
      const actionsHash = this.panel.toolbar.editor.actions
      const mlang = this.panel.toolbar.editor.mlang
      const buttonsElt = document.createElement('div')
      buttonsElt.className = 'fm-editor-section-buttons'
      for (let i = 0; i < buttons.length; i++) {
        const child = buttons[i]
        const action = actionsHash.get(child)
        if (!!action && !!action[mlang]) {
          const button = new Button(child, this)
...
;(function () {
  ...
  DropdownList.prototype = {
    ...
    addListItem(section, setActive) {
      const item = new DropdownListItem(section, this)
...
;(function () {
  ...
  Editor.prototype = {
    init() {
      ...
      document.addEventListener('click', function (e) {
        ...
        if (
          ...
          (!e.target.id.includes('table') && !e.target.id.includes('matrix'))
        ) {
    ...
    insertEquationToTextarea(equation) {
      if (this.mlang == 'latex') {
        equation = equation + ' '
      }
    ...
    createMatrixCodeLatex(rows, columns) {
      let matrixCode = ''
    ...
    createMatrixCodeMml(rows, columns) {
      let matrixCode = ''
      for (let j = 0; j < rows; j++) {
        matrixCode += '<mtr>'
        for (let i = 0; i < columns; i++) {
    ...
    renderEquationToResultarea(equation) {
      equation = equation || ''
      if (equation.trim() !== '') {
        if (this.mlang == 'latex') {
          equation = '$$' + equation.trim() + '$$'
    ...
    createResultArea() {
      const resultAreaContainer = document.createElement('div')
      resultAreaContainer.className = 'fm-editor-result-area'
      const resultAreaLabel = document.createElement('div')
    ...
    showMatrixPopup(x, y) {
      const rowsInput = document.getElementById('fm-editor-matrix-rows')
      rowsInput.value = 2
    ...
    getEquationPng(callback) {
      ...
      if (svg) {
        ...
        image.onload = function () {
          ...
          const mlang = editor.mlang
          const equation = editor.textarea.value.trim()
...
;(function () {
  ...
  MlangPanel.prototype = {
    ...
    createList() {
      ...
      const latexSection = new ButtonSection(
        { id: 'latex', name: 'LaTeX', children: [] },
        this
      )
      const latexItem = new DropdownListItem(latexSection, this)
      this.listItemContainer.appendChild(latexItem.element)
      const mmlSection = new ButtonSection(
        { id: 'mml', name: 'MathML', children: [] },
        this
      )
      const mmlItem = new DropdownListItem(mmlSection, this)
...
;(function () {
  ...
  HashArray.prototype = {
    ...
    keys() {
      const a = []
      for (const vParameter in this.obj) {
...
;(function () {
  ...
  window.DomUtils = {
    ...
    loadJsFile(filename, onLoadCallback, defaultFilename) {
    ...
    replaceSVGUseWithGraphElements(svg) {
      const useElements = svg.getElementsByTagName('use')
      const originalElements = []
      const newUseElements = []
      // Get all use elements
      for (var i = 0; i < useElements.length; i++) {
        const useElement = useElements[i]
        const originalElementId = useElement
          .getAttribute('href')
          .replace('#', '')
        ...
        for (let j = 0; j < useElement.attributes.length; j++) {
          const attribute = useElement.attributes[j]
          if (
            attribute.nodeName !== 'href' &&
            attribute.nodeName !== 'x' &&
            attribute.nodeName !== 'y'
          ) {
        ...
        if (position.x) {
          const positionStr = (position.x || 0) + ', ' + (position.y || 0)
          let transform = originalElement.getAttribute('transform') || ''
